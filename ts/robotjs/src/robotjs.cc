...
NAN_METHOD(pixelScan)
{
	...
	Local<Array> color_results = Nan::New<Array>();
	int color_count = Nan::To<int32_t>(info[info_index++]).FromJust();
	for (int i = 0; i < color_count; i++)
	{
		int x = Nan::To<int32_t>(info[info_index++]).FromJust();
		int y = Nan::To<int32_t>(info[info_index++]).FromJust();

		Nan::Set(color_results, i, Nan::New<Number>(*(pixels + y * width + x) & 0x00FFFFFF));
	}
